...
### What is Webpack
Webpack is a *static module bundler* that you can use to build JavaScript applications. This allows you to setup projects which can understand and run all sorts of files with maximum customizability including
- .js/jsx (JavaScript)
...
Webpack is setup through the configuration file, or `webpack.config.js` which will include all of our settings. There are concepts like `input` and `output` that specify where our code lives. There are also `loaders` that provide directions on what to do with different file types.
...
```
In this application so far we used the `entry`, `module`, `resolve`, `output`, `plugin`, and `devServer` settings
...
Now that we've built and run a basic **React** application in the browser, its time to prepare it for **production**.